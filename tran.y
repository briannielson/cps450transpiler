...
%token RESV_IF RESV_ELSE RESV_WHILE RESV_RET RESV_TRUE RESV_FALSE RESV_MAIN
%token TYPE_BOOL TYPE_CHAR TYPE_FLOAT TYPE_INT
%token LOG_AND LOG_OR CMP_EQUAL CMPprint_NOTEQUAL CMP_GREQ CMP_LTEQ '>' '<'
...
MulOp		: '*' | '/' | '%' {}
Factor		: UnaryOp Primary { dprintf(outfile, "Unary Op found\n"); }
			| Primary {}
UnaryOp		: '-' | '!' {}
...
Literal		: INTEGER { dprintf(outfile, "Integer found: %d\n", $1); }
			...
			| Boolean { dprintf(outfile, "boolean found\n"); };
Boolean 	: RESV_TRUE { dprintf(outfile, "TRUE"); }
			| RESV_FALSE { dprintf(outfile, "FALSE"); };
